...
def _code_functions(record_code=lambda function: function):
    """Bind diagnostic helpers to one card's recorder without shared wrappers.

    The undecorated defaults also support scripts and unit tests. Only helpers
    actually called enter the card's code listing; UI and task plumbing stay out.
    """
    @record_code
    def _numeric(series):
    ...
    return _analyze, _table, _figure, _percentiles


_analyze, _table, _figure, _percentiles = _code_functions()
...
def instance():
    ...
    def server(input, output, session):
        busy = this.busy()
        cancellation = Event()
        analyze, table_scores, figure_scores, _ = _code_functions(this.record_code)
        ...
        for i, name in enumerate(("Aggregate",) + METHODS):
            def register(i, name):
                ...
                def chart():
                    ...
                    widget = go.FigureWidget(figure_scores(Results(), view=name, top=input.Top(), full_screen=full))
