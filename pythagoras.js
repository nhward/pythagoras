...
document.addEventListener("DOMContentLoaded", () => {
    ...
    Shiny?.addCustomMessageHandler?.("SortableRoles", function(msg) {
        msg = msg || {};
        console.log(`[pythagoras] SortableRoles running for ${msg}`);
        const el = document.getElementById(msg.id);
        if (!el) {
            console.error("[pythagoras] Sortable element not found: ", msg.id)
            return;
        }
        const value = msg.value
        if (!value) {
            console.error("[pythagoras] Value not supplied: ")
            return;
        }
        ...
        function emitRoleMap() {
            ...
            window.Shiny?.setInputValue?.(value,  payload, { priority: "event" });
        }
...
// // Sortable behaviour
// console.info("[sortable] Extention-Script running (for role assignment")
