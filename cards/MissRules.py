...
def instance():
    ...
    def settings():
        return ui.TagList(
            ui.input_slider(
                id="MinSupport",
                label="Minimum permitted rule support",
                min=0.1,
                max=0.95,
                value=0.1,
                step=0.01,
                guide=this,
                text="Rules whose support is below this threshold are dropped.",
                position="left",
            ),
            ui.input_slider(
                id="MinLift",
                label="Minimum permitted rule lift",
                min=0.1,
                max=5,
                value=2,
                step=0.1,
                guide=this,
                text="Rules whose lift is below this threshold are dropped.",
                position="left",
            ),
            ui.input_slider(
                id="MaxLength",
                label="Maximum variables per rule",
                min=2,
                max=15,
                value=10,
                step=1,
                guide=this,
                text="Limits the total number of LHS and RHS variables in a rule.",
                position="left",
            ),
            ui.input_checkbox(
                id="RemoveRedundant",
                label="Prune redundant rules",
                value=False,
                guide=this,
                text="Hide specialised rules when a simpler rule with the same conclusion has at least as much confidence.",
                position="left",
            ),
            ui.input_slider(
                id="MaxObs",
