...
this.restore_configuration_state({"inputs": {"role_map": {
    "target": ["y"], "predictor": ["x2"], "identifier": ["id"],
    "weighting": ["x1"], "partition": ["part"],
    "stratifier": [], "treatment": [], "geometry": [], "sequence": [],
    "sensitive": [], "none": [],
}}})
...
this.footer = lambda: ui.TagList(
    ...
    ui.input_action_button("Convert", label="Finish upstream type restoration"),
    ui.input_action_button("Invalid", label="Incoming missing target"),
...
def scenario_server(input, output, session):
    ...
    def convert():
        source = this._imports.get()
        converted = source.frame.copy()
        converted["x1"] = converted.x1.astype(float)
        converted["y"] = [1, 0, 1, 0]
        this._imports.set(proxy_data(_df=converted, _cluster_count=3))

    @reactive.effect
    @reactive.event(input.Invalid)
    def invalid():
        changed = this._imports.get().frame.copy()
        changed.loc[0, "y"] = float("nan")
        this._imports.set(proxy_data(_df=changed, _cluster_count=3))
    ...
    def ExportProbe():
        ...
        return (f"K={data.cluster_count}; target={','.join(data.role_map.to_primitive()['target'])}; "
                f"weighting={','.join(data.role_map.to_primitive()['weighting'])}; dtype={data.frame.x1.dtype}; steps={len(data.processing_records)}")
