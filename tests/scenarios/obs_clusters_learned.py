...
source = proxy_data(_df=frame, _cluster_count=2, _name='Two groups')
scaler = StandardScaler().set_output(transform='pandas')
this._imports.set(source.with_pipeline_step(scaler, name='scale', preview_frame=scaler.fit_transform(frame)))
original_server = this.server
original_footer = this._footer
this.footer = lambda: ui.TagList(original_footer(), ui.output_text('ExportProbe'), ui.input_action_button('ChangeK', label='Change upstream K'))
