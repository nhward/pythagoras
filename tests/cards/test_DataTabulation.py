...
app = create_app_fixture(app="../../cards/DataTabulation.py", scope="function")
_HELPER_CARDS = {}
...
@pytest.fixture
def card_module():
    return importlib.import_module("cards.DataTabulation")
...
def recorded_helpers(card_module, *, data=None, decimals=2, bounded=True, fullscreen=False):
    ...
    if data is not None:
        with reactive.isolate():
            card._imports.set(
                data if isinstance(data, ProxyData) else ProxyData(_df=data, _name="Test")
            )
