...
@pytest.fixture
def card_module():
    return importlib.import_module("cards.var_dissimilar")
...
def recorded_helpers(card_module, *, frame=None, inputs=None, role_map=None, fullscreen=False):
    ...
    proxy = source if isinstance(source, proxy_data) else proxy_data(
        _df=source,
...
class TestPreparationAndStatistics:
    ...
    def test_categorical_spread_is_one_minus_modal_proportion(self, card_module):
        ...
        assert np.isnan(result.loc["group", "Skew"])

    @pytest.mark.unit
    def test_list_and_geometry_have_no_numeric_summaries(self, card_module):
        frame = pd.DataFrame({
            "basket": as_list([["a", "b"], ["a"], ["b"], None]),
            "shape": as_geometry(
                ["POINT (0 0)", "POINT (1 1)", "POINT (0 0)", None],
                representation="wkt",
            ),
        })
        _, functions = recorded_helpers(card_module, frame=frame)
        result = functions["_numeric_stats"](frame, robust=False)

        assert result.loc[
            ["basket", "shape"], ["Centrality", "Spread", "Skew"]
        ].isna().all().all()

    @pytest.mark.unit
    def test_cyclic_spread_uses_circular_dispersion(self, card_module):
        frame = pd.DataFrame({
            "concentrated": as_cyclic([0, 0, 0, 0], period=4),
            "uniform": as_cyclic([0, 1, 2, 3], period=4),
        })
        _, functions = recorded_helpers(card_module, frame=frame)
        result = functions["_numeric_stats"](frame, robust=False)

        assert result.loc["concentrated", "Spread"] == pytest.approx(0.0)
        assert result.loc["uniform", "Spread"] == pytest.approx(1.0)
        assert result[["Centrality", "Skew"]].isna().all().all()

    @pytest.mark.unit
    def test_datetime_uses_timeline_statistics(self, card_module):
        frame = pd.DataFrame({
            "when": pd.to_datetime([
        ...
        assert result.loc["when", "Centrality"] == pytest.approx(1.0)
        assert result.loc["when", "Spread"] > 0
        assert result.loc["when", "Skew"] == pytest.approx(0.0, abs=1e-12)
